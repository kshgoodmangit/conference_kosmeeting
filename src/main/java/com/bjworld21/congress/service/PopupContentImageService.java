...
@Service
public class PopupContentImageService {
    ...
    public PopupContentImageService(BoardProperties properties, UploadStorage uploadStorage) {
    ...
    public StoredContentImage store(MultipartFile file) {
        ...
        try {
            ...
            throw new IllegalStateException("팝업 이미지를 저장하지 못했습니다.", exception);
    ...
    public StoredImageResource load(String savedFilename) {
        ...
        Path target = uploadStorage.resolve(UploadStorage.POPUPS, normalizedFilename);
        if (!Files.isRegularFile(target)) {
            throw new IllegalArgumentException("팝업 이미지를 찾을 수 없습니다.");
        }
        ...
        try {
            ...
            throw new IllegalStateException("팝업 이미지 경로를 읽을 수 없습니다.", exception);
        } catch (IOException exception) {
            throw new IllegalStateException("팝업 이미지 정보를 읽을 수 없습니다.", exception);
        }
    }
    ...
    private void validateBasicFile(MultipartFile file) {
        ...
        if (file.getSize() > properties.getMaxContentImageBytes()) {
            throw new IllegalArgumentException("팝업 이미지는 파일당 5MB를 초과할 수 없습니다.");
        }
    ...
    private void deleteQuietly(Path target) {
        try {
            ...
            // 저장 실패 원인을 유지하고 남은 파일은 운영 정리 작업에서 처리합니다.
        }
    }
