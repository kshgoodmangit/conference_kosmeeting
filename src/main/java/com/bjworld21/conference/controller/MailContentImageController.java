...
import com.bjworld21.conference.service.MailContentImageService;
...
@RestController
@RequestMapping("/api")
public class MailContentImageController {
    private final MailContentImageService service;
    private final PromotionalMailProperties properties;

    public MailContentImageController(MailContentImageService service, PromotionalMailProperties properties) {
        this.service = service;
        this.properties = properties;
    }

    @PostMapping(value = "/admin/mail/images", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<Map<String, Object>> upload(@RequestPart("upload") MultipartFile file) {
        try {
            MailContentImageService.StoredContentImage storedImage = service.store(file);
            String imageUrl = publicBaseUrl() + "/api/mail/images/" + storedImage.savedFilename();
    ...
    public ResponseEntity<Resource> image(
        ...
        MailContentImageService.StoredImageResource storedImage = service.load(yearMonth + "/" + savedFilename);
