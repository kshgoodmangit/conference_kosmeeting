...
import com.bjworld21.conference.entity.MenuHtmlHistory;
...
class MenuTranslationServiceTest {
    ...
    @Test void menuUpdatesCannotClaimConfiguredLanguagePaths() {
        when(conferences.getSettings(1L)).thenReturn(ConferenceSettingsResponse.builder().seq(1L)
                .supportedLanguages(List.of("ko", "en", "ja", "zh-Hans")).defaultLanguage("en").build());
        when(menus.findBySeqForUpdate(1L, 10L)).thenReturn(menu());
        var menuService = new MenuSettingsService(menus, historyService);
        menuService.setTranslationService(service);

        for (String path : List.of("/ko", "/en", "/ja", "/zh-hans")) {
            assertThatThrownBy(() -> menuService.update(1L, 10L, "Welcome", "welcome", "page", "full", path,
