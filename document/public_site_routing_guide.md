...
`multi` 모드의 `/`는 학회 선택 화면이다. 언어 전환은 현재 페이지를 유지하며 `seq`, `page`, `category` 등 허용된 조회값만 유지한다. 비밀번호 재설정 토큰 화면에서는 언어 전환을 숨기고, 토큰을 다른 언어 링크에 복사하지 않는다.
...
기존 데이터에서 `routePath`에 폴더 경로가 남아 있고 `menuPath`에 단일 페이지명이 있으면, 사용자 메뉴 조회 시 `menuPath`로 공통 페이지 경로를 결정한다. 메뉴 링크와 컨트롤러의 메뉴 검색에 같은 경로가 사용된다. 이미 단일 경로로 저장된 `routePath`는 우선하며 외부 링크는 변경하지 않는다. 변환 결과가 같은 메뉴가 있으면 잘못된 메뉴로 연결하지 않고 설정 오류로 처리한다. DB 값은 자동 변경하지 않으며 관리자 조회에는 실제 저장값을 표시한다. 이는 기존 메뉴 데이터 읽기에만 적용되며, 이전 폴더 URL 요청의 리다이렉트나 HTML 본문의 링크 변환은 제공하지 않는다.
