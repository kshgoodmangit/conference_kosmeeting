...
export const AnalyticsTestDataCard = ({onNotify,disabled,onBusy,dashboardEnabled}:{
    ...
    const start=async()=>{
        ...
        const approved=await confirm({title:'기존 접속 데이터 삭제 후 재생성',message:`선택한 행사 #${conferenceSeq}의 기존 접속 데이터 전체(실제 접속·테스트 이벤트·일별 집계)를 삭제합니다. 삭제한 데이터는 복구할 수 없습니다. 행사 시작 60일 전부터 전날까지 동안 하루 1,000~2,000 페이지뷰, 총 6만~12만 페이지뷰를 새로 생성합니다. 체류시간 이벤트를 포함한 원본 이벤트 수는 이보다 많습니다. 다른 행사의 데이터는 유지합니다.`,confirmText:'전체 삭제 후 60일 데이터 생성',tone:'danger'});
        if(!approved)return;
    ...
    return <section className="rounded-xl border border-cyan-200 bg-white p-5 shadow-sm dark:border-cyan-900 dark:bg-slate-950">
        ...
        <p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">행사 시작 60일 전부터 전날까지 · 하루 1,000~2,000 페이지뷰(총 6만~12만) · 기기·브라우저·OS·국가·유입경로·시간대·인기 페이지·체류시간 포함</p>
